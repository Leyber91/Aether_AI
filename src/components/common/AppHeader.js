...
import { FiChevronDown, FiLogOut, FiUser, FiMessageCircle, FiEye, FiBarChart2, FiSettings, FiSliders, FiCpu } from 'react-icons/fi';
...
const ControlBar = ({ currentRoute, navigateTo }) => (
  <div className={styles['control-bar']}>
    ...
    <button
      className={currentRoute === ROUTES.DEAC ? styles['active-app'] : styles['app-switch-btn']}
      onClick={() => navigateTo(ROUTES.DEAC)}
      aria-label="DEAC System"
      title="Dynamic Evolving AI Conglomerates"
    >
      <FiCpu size={20} className={styles['nav-svg-icon']} />
      <span className={styles['nav-label']}>DEAC</span>
    </button>
    <button
